...
def threshold(img, threshold_setting):
    ...
    # global thresholding
    # blurglobal = cv2.GaussianBlur(img, (5, 5), 0)
    blurglobal = cv2.bilateralFilter(img, 9, 75, 75)
    global_threshold_val = 100
    ret1, th1 = cv2.threshold(blurglobal, global_threshold_val, 255, threshold_setting)
    images.extend(
        [
            img,
            0,
...
img = cv2.adaptiveThreshold(
    blur2, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2
)


(img_h, img_w) = img.shape[:2]
# # Apply edge detection to highlight structures like metal pins
for i in range(0, 255, 20):
    for j in range(i, 255, 20):
        edges = cv2.Canny(img, i, j)
        # Find contours in the edge-detected image
        contours, _ = cv2.findContours(edges, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        # Convert the original image to color to draw boxes
        cv_image_color = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
        # Define a list to hold bounding boxes
        bounding_boxes = []

        # Loop through the contours and draw bounding boxes for detected objects
        for contour in contours:
            x, y, w, h = cv2.boundingRect(contour)
            # Filter to ignore very small or very large boxes (noise or background)
            if 10 < w < 100 and 10 < h < 100:  # Adjust size thresholds as needed
                bounding_boxes.append((x, y, w, h))
                cv2.rectangle(cv_image_color, (x, y), (x + w, y + h), (0, 255, 0), 2)
        # cv2.imshow(f"{i}{j}", cv_image_color)
        img = cv2.resize(img, (int(img_w / 2), int(img_h / 2)))
        cv2.imshow(f"{i}{j}", img)
        cv2.waitKey(0)
        cv2.waitKey(0)
